...
public class TextViewAd extends android.support.v7.widget.AppCompatTextView
{
    ...
    private List<NoticeInfo> mTexts; //显示文字的数据源
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        switch (action)
        {
            case MotionEvent.ACTION_DOWN:
                ...
                {
                    onClickLitener.onClick(mTexts.get(mIndex).getLink());
                }
    ...
    protected void onDraw(Canvas canvas)
    {
        if (mTexts != null)
        {
            mPaintFront.setTextSize(160);
            Log.i(TAG, "onDraw: " + mY);
            //获取当前的数据
            NoticeInfo model = mTexts.get(mIndex);
            String font = model.getFrontContent();
            String back = model.getBackContent();
            // 绘制前缀的外框
